...
def profile():
    ...
    return render_template('profile.html', ads=user_ads, current_user_role=current_user_role, current_user_name=current_user_name)
...
def login():
    if request.method == 'POST':
        ...
        users = load_data('db/users.json')
        if username in users and encrypt_password(users[username]['password']) == password:
            session['user'] = {
                "username": username,
